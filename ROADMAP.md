...
- [ ] **IR → Python 代码转换**：从 IR 还原回可执行的 Python 代码
- [ ] **Graphviz / Mermaid 可视化**：输出计算图，方便调试
...
- [ ] **输出→校验→重试管道**：代码输出后自动跑语法/类型检查，失败触发约束重试
- [ ] **约束解码（Constrained Decoding）**：根据上下文限制 token 候选集，防止错误输出
- [ ] **错误检测与自愈**：检测输出的 API 是否真实存在，不存在则回退修正

### 3.3 RAG 与仓库级理解

- [ ] **轻量 Vector Store**：FAISS / Chroma / LanceDB 本地嵌入索引
- [ ] **Per-File Isolation 检索**：分析前检索当前文件依赖和局部上下文
- [ ] **仓库级代码理解**：项目级符号索引 + 依赖图，弥补小型引擎上下文短板
